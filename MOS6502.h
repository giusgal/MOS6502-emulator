...
class MOS6502 {
public:
    MOS6502(std::function<void(uint16_t, uint8_t)> w, std::function<uint8_t(uint16_t)> r);
    void IRQ();
    void NMI();
...
private:
    ...
    uint8_t currentOpCodeCycles;        //Cycles for the current opCode
    ...
    */
    ...
    /**** Comparison ****/
    void compareRM(uint8_t reg, uint8_t memory);       //Compare register with memory (Set SR flags)

    /**** Addition and Subtraction ****/
    void addWithCarry(uint8_t memory);                 //Add memory to AC with carry (and set SR flags)
    void subWithBorrow(uint8_t memory);                //Subtract memory to AC with borrow (and set SR flags)
};
